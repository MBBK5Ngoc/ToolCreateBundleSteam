...
==========================================
Usage:
    python main.py <appid1> <appid2> [options]
...
Options:
    --rotation DEGREES      Tilt the diagonal line by extra degrees (default: 0)
...
def download_assets_for(appids: list[str], downloads_dir: Path) -> None:
    ...
    # Import inline to avoid hard failure if steam package is missing
    try:
    ...
    try:
        ...
        for appid in int_appids:
            ...
            download_app_assets(
                ...
                skip_existing=True,       # skip files that already downloaded fine
            )
...
def generate_bundle(appid1: str, appid2: str,
    ...
    process(str(appid1), str(appid2), downloads_dir, output_dir,
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Steam bundle assets from two App IDs.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python main.py 3681780 2666510
  python main.py 570 730 --downloads-dir myassets --output-dir bundles
        """,
    )
    parser.add_argument("appid1", type=str, help="First game Steam App ID")
    parser.add_argument("appid2", type=str, help="Second game Steam App ID")
    parser.add_argument(
    ...
    appids = [args.appid1, args.appid2]

    print("=" * 60)
    print(f"  Steam Bundle Asset Generator")
    print(f"  App 1 : {appids[0]}")
    print(f"  App 2 : {appids[1]}")
    ...
    print("=" * 60)
    generate_bundle(args.appid1, args.appid2, downloads_dir, output_dir,
    ...
    print(f"        -> {output_dir / ('bundle_' + args.appid1 + '_' + args.appid2)}")
